...
public class ScoutManager {
	...
	public void assignScoutIfNeeded() {
		...
		if (enemyBaseLocation == null) {
			if (currentScoutUnit == null || currentScoutUnit.exists() == false
	...
	public Position getScoutFleePositionFromEnemyRegionVertices() {
		...
		// are close enough
			...
			while (distanceFromCurrentVertex < 128) {
				...
				distanceFromCurrentVertex = enemyBaseRegionVertices.get(currentScoutFreeToVertexIndex)
						.getDistance(currentScoutUnit.getPosition());
			}
		...
		for (int i = 0; i < closestTobase.size(); ++i) {
			...
			if (!surrounded && MyBotModule.Broodwar.isBuildable(tp)) {
				if (Config.DrawScoutInfo) {
					...
					MyBotModule.Broodwar.drawTextMap(x1 + 3, y1 + 2,
							"" + BWTA.getGroundDistance(tp, basePosition.toTilePosition()));
