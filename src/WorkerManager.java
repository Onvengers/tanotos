		...
		for (Unit worker : workerData.getWorkers()) {
			...
			if (workerData.getWorkerJob(worker) == WorkerData.WorkerJob.Minerals) {
				if (MyBotModule.Broodwar.self().completedUnitCount(UnitType.Protoss_Nexus) > 1
						&& StatusIndicator.Instance().getSecondCenterWorkerCount() < 10) {
					commandUtil.attackMove(worker, LocationManager.getInstance()
							.getSectionPosition(StatusIndicator.Instance().getMySectionOf(), MapSection.SECOND_CENTER));
					StatusIndicator.Instance().addSecondCenterWorkerCount();
				}
			}
