...
public class StrategyRuleSupplyProviding extends StrategyRule {

	public StrategyRuleSupplyProviding(StrategyType type) {
		super(type);
	}
	...
	public Strategy judgeStrategy() {
				...
				if (MyBotModule.Broodwar.self().supplyTotal() <= 400) {
					...
					supplyMargin += MyBotModule.Broodwar.self().completedUnitCount(InformationManager.Instance().getBasicCombatBuildingType()) * 2;
